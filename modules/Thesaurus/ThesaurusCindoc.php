 ...
 * ensuite la liste alphabétique pour compléter les termes avec les EP, EM, TA et NA.
 * 
 * Utilisation :
 * Créer un objet ThesaurusCindoc en passant au constructeur les path du fichier
 * alphabétique et du fichier hiérarchique (les deux fichiers exportés par Cindoc).
 * 
 ...
 * 
 * Remarques : quelques vérifications sont faites pour vérifier la cohérence entre 
 * le fichier alphabétique et le fichier hiérarchique. Les erreurs rencontrées sont 
 * directement affichées sur la sortie standard sous la forme 'WARNING : xxx'.
 */
class ThesaurusCindoc
{
    ...
    /**
     * Charge les listes hiérarchique et alphabétique en mémoire et trie le thesaurus
     * par ordre alphabétique.
     *
     * @param path $alphaPath le path du fichier de la liste alphabétique.
     * @param path $hierPath le path du fichier de la liste hiérarchique.
     */
    ...
    /**
     * Charge la liste hiérarchique à partir du fichier d'export de Cindoc.
     *
     * @param path $path le path du fichier de la liste hiérarchique.
     */
    ...
    /**
     * Charge la liste alphabétique à partir du fichier d'export de Cindoc.
     *
     * @param path $path le path du fichier de la liste alphabétique.
     */
    ...
    /**
     * Construit un terme (le terme et ses relations MT, TG, TS) à partir de la liste
     * hiérarchique.
     *
     * @param string|null $MT MT du terme à construire. 
     * @param string|null $TG TG du terme à construire.
    ...
    /**
     * Lit un terme de la liste alphabétique.
     * 
     * Retourne :
     * - un tableau si ok,
...
//            throw new Exception("Le terme $fre figure dans la liste hiérarchique mais pas dans la liste alphabétique structurée");
        ...
        while(substr($this->get(),0,1)===' ')
        {
            ...
            {
                case 'RE': // On ignore les termes relais, on les a récupérés via la liste hiérarchique
                    continue 2;
                    
                case 'MV': // Mot vedette = MT dans le format du thesaurus, déjà lu dans la liste hiérarchique
                    $rel='MT';
                case 'TG':  
                case 'TS':
                    if (!isset($term[$rel]))
     ...
     * @param bool $multi à true pour accepter plusieurs relations $rel pour le terme.
     */
